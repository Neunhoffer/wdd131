...
function recipeCardTemplate(recipe) {
	const tagChips = recipe.tags.map(tag => `<span class="tag">${tag}</span>`).join('');
	return `
...
function handleSearch() {
	const keyword = document.querySelector('.search-area input').value.trim();
	if (!keyword) return;
	displayRecipes(filterAndSort(keyword));
}

const randomRecipe = recipes[Math.floor(Math.random() * recipes.length)];
displayRecipes([randomRecipe]);

document.querySelector('.search-area button').addEventListener('click', handleSearch);
