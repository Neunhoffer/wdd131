...
form.addEventListener("submit", function (event) {
  ...
  if (type === "many" && note === "") {
    output.textContent = "Please add travel notes for serving at multiple campuses.";
    return;
  }

  if (type === "many" && selectedCampuses.length < 2) {
    output.textContent = "You selected 'Two or more campuses' but only chose one campus. Please select at least two campuses.";
    return;
  }
